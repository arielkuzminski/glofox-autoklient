...
(function () {
  ...
  function waitForProductButton(timeoutMs = 5000) {
    ...
    return new Promise((resolve) => {
      ...
      const timeoutId = setTimeout(() => {
        observer.disconnect();
        console.warn(
          "[GLOFOX] Timeout: przycisk produktu nie stał się aktywny.",
        );
        resolve(null);
      }, timeoutMs);
  ...
  /**
   * Otwiera modal wyboru produktu.
   * @returns {Promise<boolean>}
   */
  async function openProductModal() {
    const btn = await waitForProductButton(5000);
    if (!btn) {
      console.warn("[GLOFOX] Nie znalazłem przycisku 'Wybierz artykuł'.");
      return false;
    }
    btn.click();
    return true;
  ...
  function waitForModalReady(timeoutMs = 5000) {
    ...
    return new Promise((resolve) => {
      ...
      const timeoutId = setTimeout(() => {
        console.warn("[GLOFOX] Timeout: modal klienta nie otworzył się.");
        observer.disconnect();
        resolve(false);
      }, timeoutMs);
  ...
  function waitForClientResult(timeoutMs = 7000) {
    if (findClientResult()) return Promise.resolve(true);
    ...
    return new Promise((resolve) => {
      ...
      const timeoutId = setTimeout(() => {
        console.warn("[GLOFOX] Timeout: lista klientów nie pojawiła się.");
        observer.disconnect();
        resolve(false);
      }, timeoutMs);
  ...
  async function selectClientFlow() {
    ...
    try {
      ...
      if (!clicked) {
        console.warn("[GLOFOX] Nie udało się kliknąć wyniku klienta.");
        return;
      }

      // Otwórz modal wyboru produktu
      const productModalOpened = await openProductModal();
      if (productModalOpened) {
        console.log("[GLOFOX] Klient wybrany i modal produktów otwarty ✅");
      }
    } finally {
      isSelecting = false;
    }
